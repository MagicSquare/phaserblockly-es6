...
class GlassMarbles2State extends Phaser.State {
    ...
    constructor() {
        ...
        /**< If true a ball it's on the red path */
        this.m_GreenOverLap = false;
        /**< If true a ball it's on the green path */
        this.m_BlueOverLap = false;
        /**< If true a ball it's on the blue path */
    ...
    create() {
        ...
        this.m_GameOverText = this.game.add.text(this.game.width / 2, this.game.height / 2, 'Game Over.', {
            fontSize: '32px',
            fill: '#fff'
        });
    ...
    render() {
        ...
        if (this.m_BoundsDebug) {
            const aDebugGroups = (inPaths, inGame) => { // TODO replace inGame by calling directly this.game thanks to arrow functions
                if (inPaths) {
    ...
    createPaths() {
        ...
        let aRedSquare = this.m_RedPath.create(60, 248, 'empty_32x32');
        ...
        let aGreenSquare = this.m_GreenPath.create(320, 355, 'empty_32x32');
        ...
        let aBlueSquare = this.m_BluePath.create(671, 355, 'empty_32x32');
    ...
    checkOverlapForEachPath(inText, inGroup, inBall, inOverLapText) {
        ...
        let aBoundsBall = inBall.getBounds();

        let aResult = false;

        inGroup.forEach((inPath) => {
            let aBounds = inPath.getBounds();
    ...
    createBalls() {
        if (this.m_Balls.total < this.m_MaxBallsLenght) {
            ...
            let aBall = this.m_Balls.create(this.game.rnd.integerInRange(335, 430), 10, 'balls-animates');
    ...
    isCloseToPalyer(inPositionX) {
        const aEpsilon = 10;
        return Math.abs(this.m_Player.body.x - inPositionX) < aEpsilon;
    }

    static getStateName() {
        return "glassmarbles2";
    }
}

export default GlassMarbles2State;